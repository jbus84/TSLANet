...
class model_training(L.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler = {
            'scheduler': torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer, 
                T_max=args.num_epochs * (TRAIN_DS_SIZE // args.batch_size), 
                eta_min=1e-6  # Minimum LR
            ),
            'interval': 'step',  # Update every epoch
            'name': 'lr_scheduler'
...
def pretrain_model(resume_checkpoint_path=None, loss_type="mse"):
    ...
    model = model_pretraining(loss_type)
...
def train_model(pretrained_model_path=None, resume_checkpoint_path=None):
    trainer = L.Trainer(
